...
}
{
    ...
    const makeServerRequest = new Promise((resuelta, rechazada) => {
        // responseFromServer representa una respuesta de un servidor
        let responseFromServer = 0;

        if (responseFromServer) {
            resuelta(console.log('We got the data'));
        } else {
            rechazada(console.log('Data not received'));
        }
    });
}